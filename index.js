...
async function startchika() {
    const chika = chikaConnect({
        ...
        browser: ['Rikka MD ^_^','Safari','1.0.0'],
        auth: state
    })
  ...
  /********** FUCTION ANTI CALL **********/
    ...
    chika.sendMessage(callerId, { text: `Bloqueio automático do sistema!\nNão ligue pra o bot!\n\nEntre em contato com o proprietário para Desbloquear !`})
    chika.sendMessage(`558287515844@s.whatsapp.net`, {text: `*Denunciar bot:* Alguem ligou pra o bot`})
    await sleep(8000)
    ...
    // Get Profile Picture Group
       ...
       chika.send5ButImg(pea[0].id, `「 Alteração das configurações do grupo 」\n\nA descrição do grupo foi alterado para *${pea[0].subject}*`, `Group Settings Change Message`, wm_fatih, [])
     }
    })
    ...
    chika.ev.on('connection.update', async (update) => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output.statusCode
            if (reason === DisconnectReason.badSession) { console.log(`Arquivo de sessão inválido, exclua a sessão e verifique novamente`); chika.logout(); }
            else if (reason === DisconnectReason.connectionClosed) { console.log("Conexão fechada, reconectando...."); startchika(); }
            else if (reason === DisconnectReason.connectionLost) { console.log("Conexão perdida do servidor, reconectando..."); startchika(); }
            else if (reason === DisconnectReason.connectionReplaced) { console.log("Conexão substituída, outra nova sessão aberta, feche a sessão atual primeiro"); chika.logout(); }
            else if (reason === DisconnectReason.loggedOut) { console.log(`Dispositivo desconectado, verifique novamente e execute.`); chika.logout(); }
            else if (reason === DisconnectReason.restartRequired) { console.log("Reinicialização necessária, reiniciando..."); startchika(); }
            else if (reason === DisconnectReason.timedOut) { console.log("Tempo limite de conexão esgotado, reconectando..."); startchika(); }
    ...
    kma = gam1
    mhan = await client.prepareMessage(from, kma, image, {quoted:verificado})
